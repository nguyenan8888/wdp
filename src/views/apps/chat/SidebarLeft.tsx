...
// ** Types
import { ContactType, ChatSidebarLeftType, ChatType } from 'src/types/apps/chatTypes'
...
const SidebarLeft = (props: ChatSidebarLeftType) => {
  ...
  const renderChats = () => {
    return chats.map((chat: ChatType, index: number) => {
      const activeCondition = selectedChat?._id === chat._id
        ...
        <ListItem key={index} disablePadding sx={{ '&:not(:last-child)': { mb: 1 } }}>
          ...
          >
              ...
              >
                {chat.user?.picture ? (
                  <MuiAvatar
                    ...
                    alt={`${chat.user.lastName} ${chat.user.firstName}`}
                    sx={{
                      width: 38,
                      height: 38,
                  ...
                  >
                    {getInitials(`${chat.user.lastName} ${chat.user.firstName}`)}
                  </CustomAvatar>
                )}
              </Badge>
            </ListItemAvatar>
            <ListItemText
              ...
              secondary={
                <Typography noWrap sx={{ ...(!activeCondition && { color: 'text.secondary' }) }}>
                  {`${user?._id === chat.lastMessage.user ? 'You:' : ''} ${chat.lastMessage.content}`}
                </Typography>
              }
            />
            <Box
            ...
            >
              ...
              >
                <>
                  {chat.lastMessage ? formatDateToMonthShort(chat.lastMessage.createdAt as string, true) : new Date()}
                </>
              </Typography>
              <Chip
                color='error'
